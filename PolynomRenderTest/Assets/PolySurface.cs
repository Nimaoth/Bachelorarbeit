...
public class PolySurface : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        Debug.DrawRay(bl.origin, bl.direction, Color.green);
        Debug.DrawRay(br.origin, br.direction, Color.green);
        Debug.DrawRay(tl.origin, tl.direction, Color.green);
        Debug.DrawRay(tr.origin, tr.direction, Color.green);
    }

    public void RecalculateCoefficients(Vector3 center, Mesh mesh)
    {
        ...
        Debug.Log($"Recalculating coefficients at {center} for {mesh.name}");
        var positions = mesh.vertices;
        var normals = mesh.normals;

        (Vector3 pos, Vector3 normal)[] vertices = positions.Zip(normals, (a, b) => (a - center, b)).ToArray();
        vertices = vertices.Distinct().OrderBy(v => v.pos.sqrMagnitude).Take(Mathf.Min(vertices.Length, maxVertexCount)).ToArray();

        if (generateTestPoints) {
            foreach (var v in vertices) {
                if (UnityEngine.Random.Range(0.0f, 1.0f) < 500.0f / (float)maxVertexCount)
                    testPoints.Add(GameObject.Instantiate(testPointPrefab, v.pos + center, Quaternion.identity, testPointParent.transform));
            }
        }

        // Debug.Log($"verts: {string.Join("\n", vertices)}");

        int m = vertices.Length;

        var X = DenseMatrix.OfRows(m, 20, vertices.Select(v => Pc(v.pos)));
        var tmp = 2 * X.Transpose() * X + Sum(20, 20, vertices, v => Y(v.pos)) + 2 * largeCoefficientPenalty;
        var c = tmp.Inverse() * Sum(20, 1, vertices, v => Z(v.pos, v.normal));
    ...
    private DenseMatrix Sum(int r, int c, (Vector3 pos, Vector3 normal)[] vertices, Func<(Vector3 pos, Vector3 normal), DenseMatrix> func) {
